                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-20">
                            ...
                            <p className="mt-4 text-gray-500 font-sans">Yüklənir...</p>
                        </div>
                    ) : error ? (
                        <div className="text-center py-20">
                            <p className="text-red-500 font-sans">Sifarişləri yükləyərkən xəta baş verdi</p>
                        </div>
                    ) : order ? (
                        <>
                            {/* Status & Date */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="bg-[#FDFBF8] p-4 rounded-xl border border-[#F3E7E1] flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-xs text-gray-500 uppercase tracking-wider">Sifariş tarixi</p>
                                ...
                                <div className="bg-[#FDFBF8] p-4 rounded-xl border border-[#F3E7E1] flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-xs text-gray-500 uppercase tracking-wider">Ödəniş statusu</p>
                                        {(() => {
                                            const statusColors = {
                                ...
                                <div>
                                    ...
                                    <div className="text-sm text-gray-600 space-y-1">
                                        ...
                                        <p className="mt-2 text-xs bg-gray-50 p-2 rounded border border-gray-100 italic">
                                            {order.notes || "Xüsusi qeyd yoxdur"}
                                        </p>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="bg-[#FDFBF8] p-4 rounded-xl border border-[#F3E7E1] text-sm">
                                        <div className="flex justify-between mb-2">
                                            <span className="text-gray-500">Ödəniş statusu</span>
                                            <span className="font-medium text-green-600">{order.status === 'Paid' ? "Ödənilib" : "Ödənilməyib"}</span>
                                        </div>
                                        <div className="flex justify-between mb-2">
                                            <span className="text-gray-500">Sifariş növü</span>
                                            <span className="font-medium uppercase">{order.type}</span>
                                        </div>
                                        <div className="flex justify-between pt-2 border-t border-[#F3E7E1] mt-2">
                                            <span className="font-bold text-[#4A041D]">Ümumi məbləğ</span>
                                    ...
                                    {order.items?.map((item, idx) => (
                                        <div key={idx} className="flex items-center gap-4 p-3 bg-white border border-[#F3E7E1] rounded-xl hover:border-[#C5A059]/30 transition-all">
                                            <div className="w-16 h-16 bg-gray-50 rounded-lg overflow-hidden flex-shrink-0">
                                                <img
                                                    src={`${API_BASE_URL}/${item.product?.primaryImageUrl}`}
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-xs text-gray-500 mt-1">
                                                    Sayı: {item.quantity} · ₼ {item.unitPrice?.toFixed(2)} / ədəd
                                                </p>
                                            </div>
